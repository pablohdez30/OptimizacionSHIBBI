...
class ScrollableComboBox(ctk.CTkFrame):
    ...
    def _on_key(self, event=None):
        ...
        # Debounce filtering to avoid blocking on every keystroke
    ...
    def _open_dropdown(self):
        ...
        # Count label
        self._count_label = tk.Label(inner, text="", font=("Segoe UI", 9),
    ...
    def _resize_dropdown(self):
        ...
        w = self._entry.winfo_width() + 34

        # Height: each item ~28px, show max 15, min 3
        n_items = min(max(len(self._filtered), 1), 15)
        item_h = 28
        h = (n_items * item_h) + 24  # +24 for count label
